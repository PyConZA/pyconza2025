...
class GrantApplicationForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'request_travel': forms.CheckboxInput(attrs={
            ...
            'request_accommodation': forms.CheckboxInput(attrs={
                'class': 'form-check-input',
    ...
    def clean(self):
        ...
        if request_travel:
            if not travel_amount:
                self.add_error('travel_amount', _("Travel amount is required when requesting travel assistance."))
            if not travel_from_city:
                self.add_error('travel_from_city', _("Travel from city is required when requesting travel assistance."))
            if not travel_from_country:
                self.add_error('travel_from_country', _("Travel from country is required when requesting travel assistance."))
        ...
        if gender == 'other' and not gender_details:
            self.add_error('gender_details', _("Please provide gender details when selecting 'Other'."))
        ...
        if current_role == 'other' and not current_role_details:
            self.add_error('current_role_details', _("Please provide role details when selecting 'Other'."))
